...
def transition(state, move):
    ...
    new_state['current_matrix'] = [state["current_matrix"][i][:]
                                   for i in range(3)]
    ...
    for i in range(3):
        for j in range(3):
            if state['current_matrix'][i][j] == 0:
                ...
                number_of_directions = get_number_of_directions(
                    state, 2-player_to_move+1)
...
def play_player_vs_ai():
    ...
    while True:
        if state['current_turn'] == 1:
            while True:
                move = int(input("Enter the row: ")), int(
                    input("Enter the column: "))
...
def apply_heuristic(state):
    return get_number_of_directions(state, state['current_turn']) - get_number_of_directions(state, 2 - state['current_turn'] + 1)
...
def minimax(depth, state, maxim):
    if depth == 0 or verify_win(state, 2 - state['current_turn'] + 1) or verify_win(state, state['current_turn']):
        return apply_heuristic(state)
...
def choose_a_move_minimax(state):
    best = -10000000  # pt ca ai-ul maximizeaza
    best_move = (-1, -1)
    for i in range(3):
        for j in range(3):
            if state['current_matrix'][i][j] == 0:
                new_state = transition(state, (i, j))
                score = minimax(9, new_state, True)
                if score > best:
                    best = score
                    best_move = (i, j)
